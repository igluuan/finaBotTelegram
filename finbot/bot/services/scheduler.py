...
async def alertar_vencimentos(application):
    """Roda todo dia às 9h — avisa parcelas que vencem em 3 dias"""
    hoje_dia = datetime.now().date().day
    dias_alerta = [hoje_dia + 1, hoje_dia + 2, hoje_dia + 3]

    try:
        with crud.get_db() as db:
            parcelas = db.query(crud.Parcela).filter(
                crud.Parcela.quitada == False,
                crud.Parcela.dia_vencimento.in_(dias_alerta)
            ).all()

        # Agrupar por usuário e notificar
        usuarios = {}
        for p in parcelas:
            usuarios.setdefault(p.user_id, []).append(p)

        for user_id, items in usuarios.items():
            linhas = ["⏰ *Parcelas vencendo em breve:*\n"]
            for p in items:
                diff = p.dia_vencimento - hoje_dia
                linhas.append(
    ...
    try:
        with crud.get_db() as db:
            ...
            for g in ganhos_recorrentes:
                ...
                ).count() > 1  # > 1 pois o recorrente (original) já está salvo
...
def start_scheduler(application):
    ...
    # Alertas de vencimento (Diário às 9h)
    scheduler.add_job(alertar_vencimentos, 'cron', hour=9, minute=0, args=[application])

    # Alertas de ganhos recorrentes (Diário às 8h)
